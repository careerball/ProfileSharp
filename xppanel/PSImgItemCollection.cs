...
namespace PSUI {
	/// <summary>
	/// PSImgItemCollection represents an arbitrary number of image index
	/// mappings
	/// </summary>
	/// <remarks>
	...
	/// </remarks>
	[Serializable]
	public class PSImgItemCollection /* : ICollection */ {
		#region class PropertyChangeEventArgs
		/// <summary>
		/// <see cref="PSImgItemCollection.PropertyChange"/> event 
		/// arguments
		/// </summary>
		public class PropertyChangeEventArgs : System.EventArgs {
			...
			/// <see cref="PSImgSet"/> changed</param>
			...
			/// mapping (as opposed to the associated <see cref="PSImgSet"/>)
			/// </summary>
			public bool IsImageIndex {
				get {
					return indexChanged != PSImgItemCollection.Undefined ;
				}
			}

			/// <summary>
			/// Determine if the PropertyChange represents a change to
			/// the associated <see cref="PSImgSet"/>
			/// </summary>
			public bool IsPSImgSet {
				get {
					return !IsImageIndex ;
				}
			}
		...
		#region Fields
		/// <summary>
		/// The associated <see cref="PSImgSet"/> that has the images being mapped
		/// </summary>
		[NonSerialized]
		private PSImgSet imageSet = null ;
		...
		/// <summary>
		/// Create an <c>PSImgItemCollection</c> that holds the specified number
		/// of image mappings
		/// </summary>
		...
		public PSImgItemCollection(int numImages) {
			if (numImages <= 0) {
		...
		/// <summary>
		/// Get/Set the associated <see cref="PSImgSet"/>
		/// </summary>
		/// <remarks>
		/// When the <see cref="PSImgSet"/> changes we send a property change
		/// event with a index value of <see cref="Undefined"/>
		/// </remarks>
		[Category("Images"),Description("PSImgSet containing referenced images")]
		public PSImgSet PSImgSet {
			get {
				return imageSet ;
			}
		...
		/// image in the <see cref="PSImgSet"/>
		/// </returns>
		...
		public Image Image(int index) {
			if ((index < 0) || (index >= Count)) {
				throw new IndexOutOfRangeException("PSImgItemCollection.Image(int): index is out-of-range") ;
			}
		...
		#region Overrides
		/// <summary>
		/// Compare this <c>PSImgItemCollection</c> to another for equality
		/// </summary>
		/// <param name="obj">The other <c>PSImgItemCollection</c> to compare</param>
		/// <returns>
		/// <see langword="true"/> if the two instances have the same definition, 
		/// <see langword="false"/> otherwise
		/// </returns>
		public override bool Equals(object obj) {
			if ((obj == null) || !(obj is PSImgItemCollection))
				return false ;

			PSImgItemCollection other = obj as PSImgItemCollection ;
		...
		/// if the <see cref="PSImgSet"/> changed</param>
